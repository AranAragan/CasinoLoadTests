...
let scenarios = {
    ...
    checkMaxCreateBonusAccountWithTransferAndDeleteAccount: {
        executor: 'constant-arrival-rate',
        exec: 'createBonusWithTransferAndCloseBonusAccountTest',
        rate: 20,
        timeUnit: '1s',
    ...
    rumpingCreateBonusAccountWithTransferDuringFiveMinutesAndDeleteAccount: {
...
export function createBonusWithTransferAndCloseBonusAccountTest() {
    ...
    let responseCancel = client.invoke('bonusApp.BonusTransferService/cancel', dataCancel);
    check(responseCancel, {
        'cancel status is 0': (r) => r.status == 0,
    })

    let responseClose = client.invoke('bonusApp.BonusAccountService/close', dataClose);
    check(responseClose, {
